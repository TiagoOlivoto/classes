...
O seguinte exemplo realiza o mesmo procedimento de criação da variável categórica utilizando as funções R base `transform()` e `ifelse()`. Note que é preciso aninhar diversos \``` ifelse()`s `` para obter a mesmo resultado de `case_when()`. Com poucas condicionantes, as duas abordagens são semelhantes, mas na medida em que diversas condicionantes precisam ser utilziadas, `case_when()` se torna muito mais clara.
