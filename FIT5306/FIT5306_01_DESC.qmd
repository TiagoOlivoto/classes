...
A estatística descritiva é um ramo da estatística que aplica várias técnicas para descrever e sumarizar um conjunto de dados^[https://pt.wikipedia.org/wiki/Estat%C3%ADstica_descritiva]. Nesse material trataremos das medidas de posição e medidas de dispersão mais utilizadas para descrever um conjunto de dados (ou grupos de dados).
...
$d$, a distância do percurso
$v_1$, a velocidade média do percurso
$t_i$, o tempo de viagem do percurso
...
No R base, não existe uma função específica para a média harmônica. Pode-se utilizar, então, a função `hmean()` do pacote `metan`^[https://tiagoolivoto.github.io/metan/].
```{r}
library(metan)
hmean(c(80, 120))
